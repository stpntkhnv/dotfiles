...
заново — но в дом ни одного контейнера ни `work`, ни `pat` в принципе не
попадали, у обоих гейт в `.chezmoiignore` появился тем же коммитом, что и сам
файл (`git log -S`, `cb30909` для `work`). То, что `command -v work` (или
`pat`) изнутри контейнера всё равно находит файл и запускает его — тот же
самый файл, тот же размер в байтах, что и на хосте, — следствие другого
механизма: `distrobox enter` монтирует домашний каталог хоста внутрь
контейнера и наследует хостовый `PATH`, так что PATH внутри контейнера
буквально указывает на файл на хосте. Гейт `.chezmoiignore` на эту видимость
не влияет вообще — он про то, что лежит в доме контейнера, а не про то, что
видно через примонтированный хост.
