...
Из этого правила есть исключение, и на сегодня оно одно: `.local/bin/work`
вырезается не только по фиче, но и по окружению (`ne .env "host"`). Причина:
`work` входит в контейнеры снаружи через `distrobox enter`, которого внутри
самого контейнера нет, — команда там была бы бессмысленна. Шапка самого
файла и комментарий над блоком называют эту причину поимённо. (До 2026-08-01
здесь было второе такое же исключение, `.local/bin/pat` — оно опиралось на
вход `rbw`, живущий в домашнем каталоге контейнера, и ушло вместе с самой
командой при выпиливании Bitwarden.)
