...
`.chezmoidata.yaml`. Это касается ровно тех записей, что перечислены в
каталоге: диффинг идёт только по собственным закладкам политики (тем самым
`PolB-`/`PolF-`), а не по всему дереву Places, так что закладка, заведённая
пользователем на свою, не каталожную ссылку (см. «Настроенное руками», пункт 4),
политикой вообще не замечается и держится как обычная. Что именно происходит с
такой ссылкой при клике — тема [isolation-links.md](isolation-links.md); здесь
только то, что закладку туда кладёт и снимает политика.
...
4. **Свои закладки на ссылки вне каталога** `bookmarks:`/`contexts[].bookmarks`
   ...
   закладку обычным способом браузера. Это не то же самое, что закладки из
   раздела «Политика» выше: те приходят из каталога и автоматически
   поддерживаются политикой (правку или удаление одной из них политика
   откатит на следующем старте); эта, заведённая самим человеком, политикой не
   отслеживается вовсе — диффинг `BookmarksPolicies.sys.mjs` идёт только по
   записям с префиксом `PolB-`/`PolF-`, которые сама же создала, — и держится
   как обычная закладка Firefox/Zen. Сама механика протокола `ext+container:`
   и то, как строку собирает `zen-open`, — в [isolation-links.md](isolation-links.md).
