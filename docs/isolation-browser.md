...
отличается ровно одним: общий список `bookmarks:` в него не приезжает, только
его собственный. Рабочие адреса в пространстве, смысл которого в отсутствии
работы, были бы ровно тем, ради чего оно и заводилось, — наоборот. Правила
Space Routing у него нет и быть не может: флаг `route` живёт на записи в
`contexts:`, а `home` там не значится.
