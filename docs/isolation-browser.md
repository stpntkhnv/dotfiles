...
Это подхватывает движок Gecko сам:
`ContextualIdentityService.sys.mjs`, метод `init()`
...
1. Строка `socks5://…` не парсится вообще. Разбор происходит в
   ...
   функция `parseProxy`) регулярным выражением
...
2. Поле прокси в интерфейсе не появляется вовсе, пока не выдано отдельное
   **необязательное** право `proxy` (оно в `optional_permissions`, а не в
   ...
   «Advanced proxy settings» (`src/js/popup.js`, метод `prepare()`)
   показывает оверлей с кнопкой «Enable» и блокирует (`el.disabled = true`)
   все поля внутри панели.
...
заново при каждом старте браузера. Это подтверждает первую часть заявления
скрипта — «переустановка не пропускается из-за совпадения адреса». Вторая
часть — что пересобранный файл гарантированно подхватывается — зависит ещё
от одной, более поздней проверки той же функции, которая сравнивает номер
версии уже установленного и вновь читаемого аддона и отменяет установку при
совпадении («Installation cancelled because versions are the same») —
проверка добавлена в исправлении бага Mozilla 1715965 именно для того, чтобы
не переустанавливать `file:`-расширения без нужды. `manifest.json` нашего
расширения несёт версию `"1.0.0"` литералом и не поднимается при пересборке
(проверено — распакованный на этой машине файл несёт ту же строку). Означает
ли это, что смена одного порта или добавление контекста в `contexts:` без
других изменений действительно долетит до уже установленного расширения при
следующем старте Zen, а не будет отменено этой второй проверкой — на этой
машине не проверено вживую (не стал перезапускать реальный, рабочий Zen ради
теста); отмечено как открытый вопрос ниже, в «Почему именно так».
...
`Policies.sys.mjs` не пропускает переустановку для `file:`-адресов — это
подтверждено чтением кода. Не проверено вживую — сработает ли переустановка,
когда меняется **только** содержимое `background.js`, а версия в
`manifest.json` остаётся `1.0.0`: более поздняя правка того же кода Mozilla
(багзилла 1715965) сравнивает номер версии уже стоящего и вновь читаемого
аддона и отменяет установку при совпадении, независимо от схемы адреса.
Здесь два подтверждённых чтением кода факта соседствуют с одним
непроверенным выводом из них: возможно, при простой смене порта в
...
**Почему `context-proxy` ищет контейнер по имени, а не по id** — `userContextId`
раздаётся в порядке создания и расходится между машинами, а на одной и той же
машине — ещё и внутри одного профиля, стоит один раз удалить и заново
завести контейнер. Наблюдаемый на этой машине пример (`personal`=1,
`stellium`=6, `digi3`=7, `scratch`=8) — не синтетический, а то, что реально
получилось после обычной эксплуатации.
