...
Это вывод на профиле, созданном заново 30 июля 2026, и номера здесь идут
подряд — ровно тот случай, который описывает политика. На прежнем профиле той
же машины они шли `1 personal`, `6 stellium`, `7 digi3`, `8 scratch`: дыры
накопились от контейнеров, которые за время жизни профиля удаляли и заводили
снова. Оба состояния нормальны, и именно поэтому и `context-proxy`, и скрипт 43
ищут контейнер по имени: предсказывать номер можно, но только пока профиль
свежий, а работать надо и на том, который старше самой политики.

Расширения действительно **загрузились** — не просто лежат файлами, а активны
(сверить с пятью ожидаемыми: `@testpilot-containers`,
...
каждый space называет своё имя, а `scratch` не открывает страницу вообще.
