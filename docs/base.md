...
(`unzip`, `zip`, `pigz`), `rsync`, `tree`, `less`, документация (`man-db`,
`man-pages`), `diffutils`, `bc`, `time`.
...
вообще что-то настраивают только четыре: `starship`, `eza`, `bat`,
...
`smartmontools`, `xdg-utils`. Три пакета в списке заслуживают отдельного
слова:
...
каждая строка инициализации внешнего инструмента обёрнута в `command -v
инструмент >/dev/null &&`, а не вызывается напрямую:
