...
Гейт решает только то, что chezmoi положит при следующих запусках, а не то,
что уже есть в доме сейчас: `pat` — файл новый и в контейнер никогда не
раскладывался, так что для него это одно и то же; но `work`, у которого гейт
устроен точно так же, в контейнерах физически остался с тех пор, когда такой
проверки для него ещё не было ([how-it-works.md](how-it-works.md), раздел
«`.chezmoiignore`») — обещание «команда есть только на хосте» относится к
тому, что раскладывает chezmoi сейчас, а не к тому, что могло осесть раньше.
