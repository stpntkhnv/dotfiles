...
Personal access token организации (Azure DevOps и подобные) человек копирует
вручную с сайта в момент, когда он нужен, и вставляет туда, где он нужен, —
без посредника в виде терминальной команды. Прежняя схема целиком видна в
истории git этого репозитория, если понадобятся детали устройства (`rbw`,
`bws`, обёртка `pat`) — восстанавливать её не планируется. Разбор того, где
вообще проходит граница доступа агента к секретам сейчас, — в
[agents.md](agents.md).
...
### Не сделано: ключи не синхронизируются через базу
...
регистрировать в GitHub и Azure DevOps руками. Токены доступа эту проблему
уже пережили другим путём — их просто копируют вручную с сайта организации,
когда они нужны (раздел «Агенты и токены доступа (PAT)» выше), — а ключи нет.

Задача разобрана до конца, но не сделана сознательно. Что выяснено, чтобы не
выяснять заново:

- **У KeePassXC нет способа завести SSH-ключ записью без участия человека.**
  База защищена мастер-паролем и не разблокируется сама; чтобы положить в неё
  запись из скрипта установки, скрипту пришлось бы либо знать мастер-пароль
  заранее (то есть хранить его же), либо просить его при каждом `apply` —
  оба варианта хуже той ручной регистрации ключа, что есть сейчас.
- **`chezmoi init` не может заглянуть в базу на чистой машине** — не только
  потому, что вопросы анкеты задаются до установки пакетов, а сам `keepassxc`
  ставится позже, скриптом 20, но и потому, что даже после установки пакета
  файла базы ещё не существует: `personal.kdbx` создаётся руками, по
  чеклисту, уже после первого `apply`.
- **К моменту скрипта 82 базы тоже ещё нет** — по той же причине, что и
  выше: она появляется позже, отдельным ручным шагом. Перенести создание базы
  в начало установки, до генерации SSH-ключей, значило бы переписать порядок
  установки целиком и придумать способ создавать зашифрованный файл без
  мастер-пароля, введённого человеком, — то есть не решить задачу, а обойти
  её другим способом с той же ценой.
- **Контейнерам при этом база не нужна.** Их домашние каталоги лежат на
...
- [sync.md](sync.md) — папка `passwords`, ступенчатые версии, device ID
  Syncthing и почему его нет в каталоге фич.
