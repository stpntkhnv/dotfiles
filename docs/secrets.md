...
вложением с именем из второй колонки. Строки контекстов команда ищет
дословно; пара `SSH host …` — то же соглашение имён, но это чистый бэкап,
`ssh-restore` его не принимает и не восстанавливает (команда работает только
с именами из `contexts:`):
...
верным. Хостовая пара в базе — чистый бэкап, автоматически её никто не
восстанавливает.
...
- выкладка ступенчатая: экспорт и проверка во временном каталоге, права
  (600/644) там же, и только полный комплект переезжает `mv` в
  `~/homes/<контекст>/.ssh`; сбой или Ctrl-C до финального `mv` не трогает
  целевой каталог, прерванный `mv` чинится повтором с `-f`.
