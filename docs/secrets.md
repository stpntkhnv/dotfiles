...
истории команд. Агентам вроде Claude Code, работающим в этом терминале,
Bitwarden даёт не один вход, а два, разных по природе: токен `bws` открывает
отдельное, урезанное хранилище Secrets Manager — рабочие ключи API одного
dev-проекта — и физически не может дотянуться до личного хранилища паролей,
потому что это разные аккаунты Bitwarden. А на хосте у агента есть и второй
путь — команда `pat`, тот же `rbw`, что и у человека: если хранилище
разблокировано, `pat` отдаёт любой личный токен ровно так же, как отдала бы
человеку. Подробнее про эту разницу — ниже и в [agents.md](agents.md).
...
`claudefiles` (внешний репозиторий, [agents.md](agents.md)), а прямое
следствие того, какому аккаунту принадлежит токен в
`~/.config/bws/access-token` — машинному аккаунту Secrets Manager, у
которого просто нет прав на личное хранилище. Это граница токена, а не
запрет для агента вообще: на хосте у агента есть команда `pat`, которая
ходит через `rbw` и личный вход человека, и через неё агент читает то же
личное хранилище, что и человек — раздел «Папка PAT и команда `pat`» ниже.
Тот же вопрос с другой стороны — что именно из этого читает агент — разобран
в [agents.md](agents.md), раздел «Где кончается граница с секретами»; здесь
— что это за граница токена и откуда она берётся у самого Bitwarden.
...
Режим `-i` устроен так, что не печатает токен независимо от порядка полей в
выводе `rbw --full`: значение пароля встречается в `--full` дословно, поэтому
вырезание по содержимому работает независимо от того, в каком порядке `rbw`
печатает пароль и заметку, а не отрезает первую строку по счёту.

Открыто принятые риски использования `pat`: `pat <имя>` печатает токен на
экран и оставляет его в скроллбеке терминала — для случаев, когда это мешает,
есть `-c` (буфер обмена) и `-i` (только заметка, без токена). Но и `-c` не
бесследно: `wl-copy` кладёт значение в буфер обмена Wayland, и оно остаётся
там до следующего копирования чем угодно — токен не попадает на экран, но и
не исчезает сам по себе сразу после команды.

**Команда кладётся только на хосте, но видна и запускается из контейнера.**
Гейт стоит в `home/.chezmoiignore`, рядом с таким же для `work`. Это не
граница безопасности, и путать одно с другим не надо: фича `bitwarden` имеет
`scope: both`, `rbw` установлен в каждом контейнере, и чеклист
`zz-next-steps` там же напоминает выполнить `rbw login` — в том числе внутри
контейнера (блок с этим напоминанием открывается условием
`{{- if has "bitwarden" .enabled }}` и стоит выше гейта `{{- if eq .env "host" }}`,
который относится только к шагам ниже по файлу).

Гейт решает только то, что chezmoi положит в дом контейнера
(`~/homes/<контекст>/.local/bin`) при следующих запусках, а не то, что оттуда
видно. `distrobox enter` монтирует дом хоста внутрь контейнера и наследует
хостовый `PATH`, поэтому `pat` изнутри контейнера — тот же самый хостовый
файл (`distrobox enter digi3 -- command -v pat` покажет
`/home/stsiapan/.local/bin/pat`, а не путь внутри `~/homes/digi3`), и он
реально запускается. Падает он там не на «command not found», а на `pat:
rbw is not configured: rbw config set email <email> && rbw login`: в доме
самого контейнера нет `~/.config/rbw/config.json`, а сокет агента `rbw` в
`/run/user/1000` изнутри контейнера не виден вовсе, потому что у контейнера
свой `/run`. То же самое верно для `work` — гейт устроен точно так же, и файл
виден из контейнера по той же причине, примонтированному дому хоста, а не
потому, что физически туда попал до появления проверки по `.env`: единственный
коммит, добавивший исключение по `.env` для `work` в `.chezmoiignore`, — тот
же самый, который завёл сам `work` ([how-it-works.md](how-it-works.md),
раздел «`.chezmoiignore`»).

Причина, по которой `pat` держат на хосте, бытовая, а не про эту видимость:
вход `rbw` живёт в домашнем каталоге контейнера и умирает вместе с ним,
значит после каждого пересоздания контейнера мастер-пароль пришлось бы
вводить заново именно там, чтобы `pat` там заработал. Скопировать значение из
соседней панели терминала дешевле — а чеклист `zz-next-steps`, если кому-то
всё же нужен `pat` внутри конкретного контейнера, предлагает ровно нужный
шаг: `rbw login` в этом контейнере.
...
### Не сделано: ключи не синхронизируются через хранилище

Ключей на машине восемь, а не два: своя пара у хоста и своя пара в каждом из
трёх контейнеров, все с разными отпечатками — скрипт 82 отрабатывает в каждом
окружении отдельно. Разные ключи на контекст здесь не недосмотр, а требование
модели угроз: один и тот же публичный ключ, зарегистрированный у двух
работодателей, связывает их между собой.

Цена этого — при пересоздании контейнеров шесть ключей умирают вместе с их
домашними каталогами, и шесть публичных частей приходится заново
регистрировать в GitHub и Azure DevOps руками. Токены доступа эту проблему уже
пережили — они уехали в папку `PAT` хранилища (раздел выше), — а ключи нет.

Задача разобрана до конца, но не сделана сознательно. Что выяснено, чтобы не
выяснять заново:

- **`rbw add` не умеет заводить записи типа «SSH-ключ»** — флага типа у него
  нет, он создаёт только обычные записи. Читать такие записи `rbw` умеет
  (в модели синхронизации есть поля приватной части, публичной и отпечатка),
  создавать — нет. Значит форму хранения приватного ключа пришлось бы
  выбирать: многострочный текст в заметке, одна строка в base64 в поле пароля
  или запись, заведённая руками через веб-интерфейс.
- **`chezmoi init` не может заглянуть в хранилище на чистой машине.** Вопросы
  анкеты задаются до установки пакетов, а `rbw` ставится позже, скриптом 20.
  Вызывать шелл из шаблона можно (`output "sh" "-c" ...`, так устроен детектор
  видеокарты в `home/.chezmoi.toml.tmpl`), но звать там ещё нечего.
- **К моменту скрипта 82 вход в хранилище тоже ещё не сделан:** `rbw login`
  сейчас стоит в финальном чеклисте, то есть в самом конце установки. Чтобы
  ключи восстанавливались во время установки, вход в Bitwarden пришлось бы
  перенести в начало — а это отдельная работа, задевающая порядок установки
  целиком и режим без терминала.
- **Контейнерам при этом хранилище не нужно.** Их домашние каталоги лежат на
  хосте (`~/homes/<контекст>`), поэтому раскладывать ключи мог бы хост, прямо
  в `~/homes/<контекст>/.ssh/`. Скрипт 82 внутри контейнера такой ключ не
  тронет: он генерирует только тогда, когда файла нет.
...
| `pat` внутри контейнера отвечает `rbw is not configured` | Так и задумано: команда там видна (дом хоста примонтирован, `PATH` унаследован), но вход `rbw` живёт в доме контейнера, а не хоста, и его там ещё нет | `rbw config set email <почта>` и `rbw login` именно в этом контейнере (чеклист `zz-next-steps` подсказывает то же самое), либо просто взять значение на хосте и вставить руками |
...
- **Секреты самого Bitwarden.** Ни токен `bws`, ни мастер-пароль `rbw`, ни
  содержимое личного хранилища не хранятся и не могут храниться в этом
  репозитории — все три заводятся руками после установки, не через chezmoi.
  Токен `bws` при этом ограничен одним dev-проектом Secrets Manager и не
  может дотянуться до личного хранилища — это разные аккаунты Bitwarden; вход
  `rbw`, наоборот, открывает именно личное хранилище — и человеку, и агенту
  на хосте через `pat`, но только тому, у кого есть мастер-пароль и
  разблокированный агент. Описано выше в разделе «Три клиента Bitwarden, а не
  один» и в [agents.md](agents.md), раздел «Где кончается граница с
  секретами».
