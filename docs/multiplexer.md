...
**Незакрытая `|| true` в `first_tab_pane`.** В ветке `count == 1` извлечение
`pane_id` идёт без `|| true`, в отличие от соседних вызовов в этой же функции
— под `set -euo pipefail` отказ здесь потушил бы весь скрипт. Проверено сейчас
по коду: строка по-прежнему без защиты, ровно как описано в
[issues/2026-07-29-work-unguarded-jq.md](issues/2026-07-29-work-unguarded-jq.md).
Ревьюер не смог воспроизвести отказ на настоящих данных (те же `$panes` и
`$tab` уже успешно разобраны почти таким же фильтром строкой выше, чтобы
получить `count == 1`), поэтому фикс отложен, а не внесён; правка кода — не в
...
- [issues/2026-07-29-work-unguarded-jq.md](issues/2026-07-29-work-unguarded-jq.md)
  — незакрытая `|| true` в `first_tab_pane`.
