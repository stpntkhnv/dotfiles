...
5. Читает карту устройств из `~/.config/syncthing-devices.yaml` (через
   `/usr/bin/yq`; файла нет, yq не установлен или файл не парсится — скрипт
   говорит об этом и завершается нулём, см. «Карта устройств: отдельный
   файл» ниже) и ищет в ней свой ID — так скрипт узнаёт, **какая он машина**
...
же ошибка, которую `docs/desktop.md` разбирает для файлов, которые
генерирует DMS поверх niri (раздел [«Что подключено, а что прошито в самом
config.kdl»](desktop.md#что-подключено-а-что-прошито-в-самом-configkdl)):
держать генерируемый файл одновременно под git и под живым владельцем —
значит переписывать его обратно на старое содержимое при каждой правке с
той стороны. Тот же документ показывает и обратный, ещё не сработавший
случай той же ловушки — файл, который выглядит «нашим», но в любой момент
может перестать им быть: [«Курсор — не такое надёжное «наше», как
выглядит»](desktop.md#курсор-не-такое-надёжное-наше-как-выглядит). Вместо
файла скрипт правит объекты по одному через REST API демона: устройство,
опцию, папку, список игноров — каждый отдельным вызовом, и только то, что
описано в каталоге.
...
устройств (`~/.config/syncthing-devices.yaml`, не в каталоге
фич — см. «Карта устройств: отдельный файл» ниже). Совпадения нет —
...
Скрипт правит только семь ключей опций демона (`desired_options`,
`home/.chezmoiscripts/run_after_46-syncthing.sh.tmpl`, строки 153-160) —
PATCH трогает именно эти ключи и ничего больше, всё остальное, что выбрал
демон или сам пользователь через GUI, остаётся как есть:

| Опция | Значение | Зачем |
|---|---|---|
| `localAnnounceEnabled` | `true` | искать другие машины в локальной сети напрямую |
| `globalAnnounceEnabled` | `false` | не объявлять себя серверам обнаружения Syncthing в интернете |
| `relaysEnabled` | `false` | не ходить через чужие релеи, если прямое соединение не удалось |
| `natEnabled` | `false` | не пытаться пробить NAT через UPnP/NAT-PMP на домашнем роутере |
| `urAccepted` | `-1` | явный отказ от анонимной телеметрии использования — это осознанное «нет», а не `0` («спросить позже»), которое демон обычно ставит сам |
| `crashReportingEnabled` | `false` | не отправлять отчёты о падениях разработчикам Syncthing |
| `startBrowser` | `false` | не открывать вкладку браузера при каждом запуске службы |

Значения сверены построчно с массивом в скрипте, а не унаследованы из
старого текста без проверки: расхождений с кодом не нашлось. Первые четыре
строки — прямое следствие того, что обнаружение машин
ограничено локальной сетью и общим [tailnet](network.md) (сеть Tailscale,
которая соединяет эти машины) и больше ничем: `localAnnounceEnabled` ищет
другую машину напрямую, пока обе в одной локальной сети, а глобальное
объявление, релеи и обход NAT выключены, потому что эти машины нигде за
пределами своей локальной сети и tailnet светиться не должны. `urAccepted
-1` и комментарий рядом с ним в самом скрипте объясняют, почему это не
«спросить позже»: демон интерпретирует `0` как временную отсрочку вопроса, а
`-1` — как окончательный отказ, который он больше не переспрашивает.
`startBrowser: false` защищает от одной конкретной мелочи: без этого ключа
перезапуск службы (например, после `PATCH`, который требует рестарта)
открывал бы вкладку браузера сам по себе.
...
Спаривание взаимно: ни одна машина не может завершить его в одиночку.

1. На новой машине — `chezmoi apply`. Скрипт создаёт identity Syncthing,
   стартует службу и завершается; `run_after_zz-next-steps.sh.tmpl`
   печатает device ID этой машины.
2. Этот ID добавляется записью в `~/.config/syncthing-devices.yaml` на
   каждой уже работающей машине (чеклист печатает готовый к вставке скелет
   записи). Это
   единственный шаг во всей процедуре, где действие на одной машине требует
   действия на другой, и пропустить его — не значит получить ошибку: обе
   стороны просто выглядят настроенными и никогда не соединяются.
3. `chezmoi apply` на каждой из уже работающих машин.
4. Полная карта устройств, все машины разом, переносится на новую машину, и
   там `chezmoi apply` запускается снова.
5. Телефон добавляется руками, с обеих сторон — см. ниже.

### Телефон

Syncthing на Android chezmoi вообще не трогает — он настраивается один раз,
руками, в самом приложении:

1. Подключиться к tailnet.
2. Добавить компьютер как устройство, указав его device ID.
3. Создать `personal` и `camera` как папки с точно такими же `id`, как в
   каталоге. Опечатка здесь не считается ошибкой: Syncthing её примет и
   молча заведёт вторую, никак не связанную папку.
4. Выставить `camera` в Send Only — зеркально хостовому `receiveonly`.
5. Оставить `personal` двусторонней.

## Что ставится и что меняется

| Категория | Путь / пакет | Где | Когда меняется |
|---|---|---|---|
| Пакет | `syncthing`, `curl`, `go-yq` | хост | ставятся при выбранной фиче `syncthing`; `curl` перечислен явно, хотя его и так тянут git с pacman, — REST-скрипт зависит от него напрямую; `go-yq` читает карту устройств (`/usr/bin/yq`, полный путь — чтобы чужой python-yq в PATH не подменил) |
...
| Конфиг вне chezmoi | карта устройств | `~/.config/syncthing-devices.yaml`, по машине | правится руками на каждой машине; chezmoi её не раскладывает, скрипты читают в рантайме (см. «Карта устройств: отдельный файл») |
...
| `run_after_zz-next-steps` пишет, что машина не в карте устройств | Device ID этой машины ещё не вписан в `~/.config/syncthing-devices.yaml` — либо впервые, либо после переустановки, когда identity сгенерировалась заново | добавить запись (next-steps печатает готовый скелет с ID) на **всех** уже работающих машинах и применить `chezmoi apply` там тоже — это единственный шаг всей настройки, где действие на одной машине требует действия на другой |
| `46-syncthing` пишет `device map exists but did not parse` | `~/.config/syncthing-devices.yaml` битый: ошибка YAML или корень файла — не объект (строка, список) | поправить файл; проверить руками: `/usr/bin/yq -o=json . ~/.config/syncthing-devices.yaml` |
| `46-syncthing` пишет `go-yq is not installed` | Пакет `go-yq` ещё не установлен — карта нечитаема без него | `chezmoi apply` ставит его сам (фича `syncthing`); либо `sudo pacman -S --needed go-yq` |
| `46-syncthing` пишет `device <имя>: entry is missing id or addresses, skipping` | У записи в карте не хватает обязательного поля — умолчаний нет, `addresses` обязателен так же, как `id` | дописать поле; машинам — `["dynamic", "tcp://<tailnet-имя>:22000"]`, телефону достаточно `["dynamic"]` |
...
### Карта устройств: отдельный файл

Устройства с их ID и адресами живут в `~/.config/syncthing-devices.yaml` —
локальном файле каждой машины, которого в git нет. Причин две, и они
разные.

**Почему не в каталоге фич.** `home/.chezmoidata.yaml` — файл этого
репозитория, который лежит в открытом виде. Device ID — устойчивый
идентификатор конкретной существующей машины; опубликовать его вместе с
адресом и вместе с тем, сколько всего таких машин, значит отдать корреляцию
бесплатно, ничего не получив взамен.

**Почему не в `chezmoi.toml`, где карта жила раньше.** Единственное место,
которое человек правит руками при подключении устройства, не должно
требовать знания TOML-таблиц chezmoi: отдельный YAML-файл со
шпаргалкой-шапкой (какое имя допустимо, где взять ID, какие адреса писать)
читается и правится без залезания в конфиг
chezmoi вообще. Схема записи — та же, что была: `id` и `addresses`,
оба обязательны; умолчания для `addresses` нет намеренно — глобальный
discovery в этой сети выключен, и «dynamic» сам по себе работает только
внутри одной локальной сети, молчаливое умолчание давало бы правдоподобно
выглядящую, но не соединяющуюся настройку. Каждому устройству, включая
телефон (он тоже в tailnet), рядом с `dynamic` пишется его
tailnet-адрес; запись без рабочего `tcp://` чеклист называет поимённо.

Оба потребителя — скрипт 46 и чеклист `zz-next-steps` — читают файл в
рантайме через `/usr/bin/yq` (пакет `go-yq`; полный путь — защита от
несовместимой python-реализации yq раньше в PATH). Каждое ненормальное
состояние файла — своя строка и выход нулём, apply не падает никогда:
нет файла (скрипт печатает ID машины, чеклист — готовый скелет записи),
нет yq, файл не парсится, корень — не объект. Запись без обязательного
поля стоит одного устройства, не всего apply.
...
У той же развилки есть контейнерное продолжение: пустой `kb.vault_path` в
контейнере раскрывается от контейнерного `$HOME` и молча заводит **второй**
волт в `~/homes/<контекст>/Documents/Notes/kb` — вне синхронизации и
смертный вместе с домом контейнера. Живой случай 1 августа 2026, с двумя
слоями причины и проверкой, разобран в
...
- [docs/desktop.md](desktop.md) — та же ловушка «два владельца переписывают
  файл друг за другом», разобранная на примере `config.kdl` и DMS: разделы
  [«Что подключено, а что прошито в самом
  config.kdl»](desktop.md#что-подключено-а-что-прошито-в-самом-configkdl) и
  [«Курсор — не такое надёжное «наше», как
  выглядит»](desktop.md#курсор-не-такое-надёжное-наше-как-выглядит).
