...
старого текста без проверки; `docs/features.md` (раздел `syncthing`)
называет ровно те же семь ключей и значения — здесь расхождений с кодом не
нашлось. Первые четыре строки — прямое следствие того, что обнаружение машин
...
- [README.md, раздел Sync](../README.md#sync) — исходный текст, из которого
  в этот документ перенесены таблица папок и обе процедуры («New machine»,
  «Phone»); при расхождении с этим документом верить этому документу, он
  сверен с кодом позже.
