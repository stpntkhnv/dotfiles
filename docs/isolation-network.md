...
before acting on it», строки 20-41):
...
|---|---|
| Дубль имени | Оба контекста получили бы один и тот же каталог `$SOCKDIR/<имя>` и одну и ту же секцию `--volume` в `distrobox.ini.tmpl` (том тоже ключуется по имени). Два разных контейнера писали бы в один и тот же файл `socks.sock`, и какой из них ответит браузеру — вопрос гонки процессов при старте, не конфигурации. Это ровно та межконтекстная утечка, ради предотвращения которой существует вся эта архитектура: браузер думает, что говорит с `digi3`, а фактически может говорить с сетью соседнего контекста |
| Дубль порта | Оба юнита `wsproxy-<имя>.service` попытались бы забрать один и тот же TCP-порт. Стартует первый по очереди systemd, второй проигрывает `bind()` (адрес занят) и остаётся в цикле `Restart=always`/`RestartSec=2`, не поднимаясь никогда. Firefox по-прежнему настроен на этот порт — значит либо тишина (контекст без прокси вообще), либо, что хуже, трафик уходит через контейнер контекста-победителя, который на этот порт и ответил первым |
| Имя `scratch` | `scratch` — зарезервированное имя Firefox-контейнера без прокси, «for links that belong to nobody» (см. [isolation.md](isolation.md)). Если бы `contexts:` завёл distrobox-пару с этим именем, скрипт честно построил бы ей мост и сокет — то есть выдал прокси и сеть контейнеру, у которого по определению не должно быть ни того ни другого |
...
| `chezmoi apply` останавливается с `!! Bad contexts in .chezmoidata.yaml` | Дубль имени, дубль порта или контекст с именем `scratch` в `contexts:` | Поправить `home/.chezmoidata.yaml`, см. таблицу проверок выше — что именно сломается, зависит от того, какая из трёх строк напечаталась |
