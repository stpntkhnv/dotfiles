...
тем же слиянием тоже обновлён: его проза описывает сокетный механизм и прямо
называет `distrobox-host-exec` историей, которая на этой машине не работала ни
разу. Устарела там только схема mermaid в начале раздела 10 — она всё ещё
рисует вход 2 через `/run/.containerenv` и `zen-open` на хосте. Правка
`BROWSER-ISOLATION.md` — вне рамок этого документа, и сам файл всё равно
уходит: его заменяет набор документов про изоляцию, частью которого является
этот.
