...
подключён. Это расхождение с более ранними текстами этого репозитория: они
называли `empty_windowrules.kdl` файлом «правил окон» без единого слова о
том, что он не подключён, хотя `wpblur.kdl` в числе файлов, которые
генерирует DMS, всё-таки называли — этот пробел теперь закрыт.

### Курсор — не такое надёжное «наше», как выглядит

Более ранние тексты этого репозитория относили `dms/cursor.kdl` к «нашим»
файлам — наравне с `binds.kdl`, в отличие от подключённых файлов, которые генерирует
