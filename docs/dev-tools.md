...
плагинов `lazy.nvim`, а дальше просто перечисляет 15 групп через `{ import
= 'plugins.X' }` — по одной на файл в `lua/plugins/`. Группы (комментарии
в самом `plugins.lua`): тестирование; внешний вид (цветовая схема,
...
привязанное решение .NET, конкретный Azure tenant, сохранённые
подключения MSSQL. `keybindings.json` — ровно одна привязка: `Shift+Enter`
