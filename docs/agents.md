...
нет — есть только флаг `--cgroup-parent` на отдельный контейнер, которым
контейнеры distrobox, создаваемые самим repo, не достать. Бюджет положен
туда, где контейнеры уже есть. Контейнеры Aspire с 2026-08-03 создаются
вложенным podman'ом внутри рабочего контекста
([nested-podman.md](nested-podman.md)) — то есть сидят в cgroup своего
контейнера, под его `--memory=8g` и под этим же общим бюджетом: повторение
аварии 2 августа теперь упирается сначала в потолок контекста, а не сразу в
своп машины.
