...
и в контейнере она разная. Агент **внутри контейнера** до личного хранилища
не дотягивается: команды `pat` там нет ([secrets.md](secrets.md)), а `rbw`
хоть и установлен, но входа в нём нет, пока человек не выполнит `rbw login`
руками именно в этом контейнере. Агент **на хосте** личное хранилище
прочитать может: там `pat` есть, и при разблокированном `rbw` она отдаст
любой токен из папки `PAT`. Это следствие того, что хост принадлежит человеку
целиком, а не отдельная защита, и писать иначе было бы самообманом.
