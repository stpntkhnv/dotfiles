...
Секретов, к которым агент имел бы канал, на этой машине больше нет ни
одного, ни на хосте, ни в контейнере — 2026-08-01 это стало проще, чем было.
До этой даты граница проходила через два разных клиента Bitwarden: токен
`bws` открывал урезанное хранилище Secrets Manager, а команда `pat` на
хосте ходила через `rbw` и личный вход человека, то есть агент на хосте мог
прочитать личное хранилище паролей, а в контейнере — не мог, потому что вход
`rbw` жил в домашнем каталоге контейнера, а не хоста. Вся эта схема выпилена
вместе с самим Bitwarden — не сужена, а убрана целиком: ни `rbw`, ни `bws`,
ни `pat` на машине больше нет.

Пароли теперь живут в KeePassXC — локальном файле на хосте, к которому у
терминала нет никакого канала: нет ни CLI-обёртки вроде `pat`, ни токена
вроде `bws`, а сам файл базы зашифрован под мастер-паролем, который никто,
включая агента, не вводит автоматически. Personal access token организаций
человек копирует вручную с сайта организации в момент, когда он нужен, — то
есть агент получает такой токен, только если человек сам вставит его в
разговор, а не потому, что агент умеет пойти и прочитать его сам. Устройство
