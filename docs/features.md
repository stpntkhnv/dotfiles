...
| 37 | after | onchange | только контейнер | Обёртка `/usr/local/bin/xdg-open`: веб-ссылки уходят на хост с именем контекста | да |
| 38 | after | onchange | хост + `zen` | `/usr/local/bin/zen-open`, `.desktop` на каждый контекст, Junction обработчиком по умолчанию | да |
...
flowchart LR
    ...
    subgraph USR["/usr/local"]
        U1["bin/zen-open"]
        U2["bin/xdg-open (обёртка)"]
        U3["bin/killswitch-apply"]
        U4["lib/zen-context-proxy.xpi"]
        U5["share/kbd/keymaps/us-swapcaps.map"]
    end
...
| `37-container-links` | В контейнере: подменяет `xdg-open`, чтобы веб-ссылки уходили на хост с именем контекста |
| `38-linkrouting` | На хосте: `zen-open`, `.desktop` для каждого контекста, Junction обработчиком по умолчанию |
...
Из контейнера цепочка та же, только начинается с подменённого `xdg-open`, который читает имя контекста из `/run/.containerenv` и зовёт `zen-open` на хосте через `distrobox-host-exec`. Неверные аргументы проваливаются в настоящий `xdg-open`, поэтому PDF по-прежнему открывается просмотрщиком внутри контейнера.
