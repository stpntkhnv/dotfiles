...
шелл: тот же QML, те же компоненты, тот же `settings.json`. Расходиться там
нечему по устройству, а не по дисциплине - в отличие от любой темы для sddm,
которая была бы второй реализацией того же вида.
