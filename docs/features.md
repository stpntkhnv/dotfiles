...
**sddm никуда не девается.** `30-system` включает его как и раньше,
безусловно, а переключение делает только скрипт `45`. Значит рабочий вход у
машины есть всегда, а не в результате удачного стечения обстоятельств. Пакет
остаётся установленным: это то, к чему возвращает `dms greeter uninstall`.
...
считается только при трёх условиях сразу - юнит включён, бинарь `dms-greeter`
на месте и в `config.toml` есть строка `command` с ним. Включённый юнит с
пустым конфигом это чёрный экран, а не вход, и ровно это остаётся после
