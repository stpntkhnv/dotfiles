...
| **Файлы** | нет управляемых; скрипт пишет `/etc/greetd/niri_overrides.kdl` |
...
**Раскладка.** Обёртка `dms-greeter` включает `/etc/greetd/niri_overrides.kdl`
в конфиг niri, которым гритер и рисуется. Скрипт кладёт туда `us,ru` с
`ctrl:swapcaps` - третью копию той же строки, что в `niri/config.kdl` и в
`30-system`.
...
| 45 | after | **каждый раз** | хост + `greeter` | Раскладка в `niri_overrides.kdl`, передача входа гритеру, перенос темы | да |
