...
Документ описывает состояние на коммите `cd5d902`.
...
Список тех, что гоняются всегда: `43-zen-session`, `44-handy-settings`, `46-syncthing`, `80-niri-dms-placeholders`, `84-claudefiles`, `zz-next-steps`.
...
26 фич можно выбрать (34 в каталоге минус 8 с `always: true`, см. раздел 4). Чеклист фильтрует их по `scope`: на хосте видно 25, в контейнере - 18. Пробел переключает, стрелки двигают, Enter подтверждает.
...
#### `greeter` - экран входа тем же шеллом

| | |
|---|---|
| **Пакеты** | `greetd` + AUR `greetd-dms-greeter-git` |
| **Файлы** | нет управляемых |
| **Скрипт** | `45-greeter`, гоняется на **каждом** apply |
| **Условия** | `scope: host` |

Гритер DankMaterialShell это не тема, похожая на рабочий стол, а тот же самый
шелл: он читает тот же `settings.json`, что и рабочая сессия - это
единственное, чему там расходиться действительно нечему, а не вопрос
дисциплины, в отличие от любой темы для sddm, которая была бы второй
реализацией того же вида. QML при этом не буквально общий: гритер ставится
отдельным пакетом, `greetd-dms-greeter-git`, со своим путём обновления (см.
ниже) - отдельно от `dms-shell` самой сессии. Тот же апстрим-проект,
установленный дважды, а не общий код на диске.

**Почему не `always`.** Какой экран входа стоит на машине - предпочтение, как и
выбор мультиплексора. Плюс `always` затащил бы AUR-пакет на `-git` на каждую
машину, а его провал при сборке роняет `20-packages` вместе со всем apply.

**sddm никуда не девается.** `30-system` включает его только пока
`display-manager.service` ничем не занят - этот алиас потом держит greetd;
переключение делает только скрипт `45`. Значит рабочий вход у машины есть
всегда: на свежей машине sddm посеян с самого начала, а дальше на каждом
apply за это отвечает `trap EXIT` внутри `45`, который возвращает sddm, если
greetd обслужить вход не может. Пакет остаётся установленным: это то, к чему
возвращает `dms greeter uninstall`.

**Раскладка.** Раньше скрипт держал третью копию раскладки в
`/etc/greetd/niri_overrides.kdl` - вслед за `niri/config.kdl` и `30-system`,
на живой машине это оказалось лишним и небезопасным. `dms greeter sync` сам
читает раскладку из живого `~/.config/niri/config.kdl` и кладёт её в
`/etc/greetd/niri/dms.kdl`, так что третьей копии не нужно вовсе. Хуже того,
обёртка `dms-greeter` подключает override-файл после этого сгенерированного,
и в конфиге niri гритера оказывались два блока `input` разом - `niri
validate` это молча пропускает, а если для повторяющегося блока побеждает
последний, то override тихо стирал `numlock` и всё остальное, что перенёс
DMS. Скрипт больше этот файл не пишет, а старую копию, если найдёт её по
метке `Managed by chezmoi` в первой строке, удаляет. Вместо записи он теперь
проверяет: после каждой синхронизации ищет блок `xkb` в `/etc/greetd/niri/`
и, если не находит, громко говорит об этом - иначе неверная раскладка
вскрывается только на поле пароля.

**Спасательный люк.** Проверка «в эту машину можно войти» висит на `trap EXIT`,
поэтому выполняется на любом пути выхода, включая аварийный, и она же
принудительно возвращает нулевой код: незащищённый `sudo` под `set -e` иначе
уронил бы весь apply, хотя сама проверка отработала. Рабочим входом greetd
считается только при четырёх условиях сразу - юнит включён, бинарь
`dms-greeter` на месте, в `config.toml` есть строка `command` с ним и
существует каталог `/var/cache/dms-greeter`: без него сам обёрточный скрипт
вендора отказывается стартовать. Включённый юнит с пустым конфигом или без
этого каталога - чёрный экран, а не вход, и ровно это остаётся после
`dms greeter enable`, умершего на полпути. Если рабочего входа не осталось,
скрипт возвращает sddm и говорит громко.

**Задержка темы.** Смена обоев доезжает до экрана входа на следующем
`chezmoi apply` либо по руками набранному `dms greeter sync`: синхронизация
копирует файлы, а не ссылается на них. Скрипт держит отметку
`~/.cache/dms-greeter-sync.stamp`, чтобы не звать копирование с повышением прав
на каждом apply. Слепо верить этой отметке приходится, пока
`/var/cache/dms-greeter` нельзя посмотреть: каталог `0750 greeter:greeter`, а
членство в этой группе вступает в силу только со следующим входом в систему.
Кэш, опустошённый в это окно, остаётся незамеченным - чинится это руками
набранным `dms greeter sync`.

**`DMS_PRIVESC`.** И `dms greeter enable`, и `dms greeter sync` при первом
запуске на машине сами спрашивают, каким способом повышать права - `sudo`
или `run0` - и флаг `-y` этот вопрос не глушит: он гасит только вопросы,
которые идут после этого. Без `DMS_PRIVESC=sudo` перед обеими командами
`chezmoi apply` без диалога виснет именно здесь навсегда. Если apply молча
завис на скрипте `45` - ищи именно этот вопрос.
...
| **Модель** | Whisper Large v3 Q8_0, 1669 МБ по каталогу Handy (Turbo Q8_0 - 886 МБ); закрепляет `44-handy-settings` |
...
| **Скрипты** | `40-voice` (только печатает подсказку), `44-handy-settings` (правит настройки Handy на каждом apply), `35-nvidia` косвенно |
...
**Про пунктуацию.** Whisper выбирает стиль расшифровки на первом токене каждого 30-секундного окна декодирования и держит его до конца окна; без подсказки заметная доля диктовок возвращается без знаков препинания. Единственный рычаг Handy - `initial_prompt`, а его единственный источник - список `custom_words`, куда UI не пускает фразы с пробелами. `44-handy-settings` пишет туда затравку напрямую через `jq`, в обход этого ограничения, и держит её последним элементом списка: Handy склеивает `custom_words` через `", "`, и затравка, оборванная серединой перечисления, пунктуирует хуже, чем отсутствие подсказки вовсе - это измерено. Модель заодно сменилась с Turbo на Large v3 - не ради пунктуации (тут разницы нет), а ради написания англицизмов: с затравкой Turbo пишет `Chizmoi`/`GEMA`, Large v3 - `chezmoi`/`Gemma`. Плата - задержка: у large-v3 с затравкой 622 мс против 272 у turbo с затравкой, той конфигурации, которую она заменила (RTF 36.6 против 89.6). Подробности и числа замера - в README, раздел [Punctuation](../README.md#punctuation).

**Горячие клавиши** (`voice.kdl`):

| Комбинация | Действие |
|---|---|
| `Mod+Shift+D` | диктовка (`handy --toggle-transcription`) |

**Почему через CLI, а не сигналы.** Handy умеет принимать SIGUSR1/SIGUSR2, но эти сигналы непригодны: сборщик мусора WebKit внутри того же процесса владеет SIGUSR1. `pkill -USR1` роняет Handy 0.9.4 в libjavascriptcoregtk раньше, чем тот успевает записать в лог получение сигнала. Флаг `--toggle-transcription` доходит до того же действия, не трогая путь сигналов.
...
| 44 | after | **каждый раз** | `voice` | Правит 13 ключей в настройках Handy | нет |
| 45 | after | **каждый раз** | хост + `greeter` | Уборка устаревшего override, передача входа гритеру, перенос темы, проверка раскладки | да |
...
`sddm.service`, `NetworkManager.service`, `bluetooth.service`, `ufw.service`, `systemd-timesyncd.service`, `fstrim.timer`, плюс по фичам: `cups.socket`, `tailscaled.service` (со стартом), `docker.socket`.
...
flowchart LR
    subgraph ETC["/etc"]
        ...
        E14["greetd/config.toml"]
        E15["greetd/niri/ - раскладку сюда пишет DMS"]
    end
    ...
    subgraph VAR["/var/cache"]
        V1["dms-greeter/ - копия темы для экрана входа"]
    end

    style ETC fill:#2d3748,color:#fff
    style USR fill:#2d3748,color:#fff
    style OPT fill:#2d3748,color:#fff
    style VAR fill:#2d3748,color:#fff
```
...
Скрипт `44-handy-settings` показателен:

```mermaid
flowchart TD
    A["trap на cleanup,<br/>ДО mktemp"] --> B["Собрать новый конфиг<br/>во временный файл РЯДОМ с целевым"]
    B --> C{"jq отработал?"}
    C -->|нет| D["Упасть здесь<br/>Handy ещё жив, файл цел"]
    C -->|да| E{"Результат отличается<br/>от текущего?"}
    E -->|нет| F["Выйти, ничего не трогая"]
    E -->|да| G{"handy правда запущен?"}
    G -->|нет| K["mv -f - переименование<br/>в пределах одной ФС"]
    G -->|да| H["HANDY_STOPPED=1<br/>pkill handy"]
    H --> I{"процесс исчез<br/>за ~10с?"}
    I -->|нет| J["Не трогать файл,<br/>HANDY_STOPPED=0, exit 0"]
    I -->|да| K
    K --> L["trap на выходе<br/>перезапускает Handy,<br/>только если есть графическая сессия"]
...
Детали, каждая из которых была ошибкой:
...
2. **`trap cleanup EXIT` ставится ДО `mktemp`, а не после.** `mktemp` уже кладёт файл на диск; сбой в `chmod --reference` или сигнал в промежутке между `mktemp` и установкой trap раньше оставлял нулевой `.settings_store.XXXXXX` без всякой уборки. `cleanup` терпит `$NEW` ещё не установленной (`rm -f "${NEW:-}"`).
...
4. **`pkill` только когда Handy действительно запущен, и с ожиданием исчезновения.** `pkill -x handy || true` сам по себе не отличает "процесса и не было" от "убил" - раньше `HANDY_STOPPED=1` ставился безусловно, и trap перезапускал Handy, которого пользователь сам закрыл. Теперь флаг ставится только после `pgrep -x handy` подтвердил, что процесс есть, и скрипт ждёт до ~10 секунд короткими `sleep`, пока он не исчезнет по-настоящему: Handy подключает tauri single-instance plugin, и перезапуск, начатый пока старый процесс ещё умирает, получает отказ от этого плагина. Если за 10 секунд Handy так и не исчез, файл остаётся нетронутым, флаг сбрасывается и скрипт выходит кодом 0 - упрямый Handy не должен ронять весь `chezmoi apply`, следующий apply повторит попытку.

5. **Перезапуск в `cleanup` смотрит на `WAYLAND_DISPLAY`/`DISPLAY`.** Скрипт наследует окружение той оболочки, что его запустила. Без графической сессии в этом окружении (например, `chezmoi apply` через SSH к рабочему столу, где Handy реально работает) слепой перезапуск породил бы процесс, которому нечем достучаться до композитора - молча, потому что он уходит в фон с выводом в `/dev/null` внутри `|| true`. При пустых `WAYLAND_DISPLAY` и `DISPLAY` скрипт вместо этого печатает, что не перезапускает Handy, и оставляет его пользователю запустить руками.
