...
6. Дальше собирается желаемое состояние: строится промпт (см. «Промпт
   ...
   временный файл**, а не поверх `settings_store.json`. Патч на массиве
   `post_process_providers` устроен через `select(.id == "custom")`, а не по
   индексу: это массив объектов, порядком которого владеет Handy, и он может
   измениться между версиями. Оба массива (`post_process_providers`,
   `post_process_prompts`) при сборке подстрахованы `// []`: смена схемы Handy
