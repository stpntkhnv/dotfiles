...
Пара не обязана существовать в обе стороны. У Firefox-контейнера `scratch`
distrobox-пары нет вообще — это зарезервированное имя для ссылок, которые не
принадлежат ни одной работе. Комментарий над блоком `contexts:` в
`home/.chezmoidata.yaml` называет его прямо: «no-proxy Zen container for links
that belong to nobody». У него намеренно нет прокси, и его трафик уходит прямо
с хоста.
...
`scratch` моста нет вообще, цепочка обрывается на Zen: трафик уходит прямо с
хоста (раздел выше).
...
на хосте, Firefox-контейнер, space и папка закладок, пункт в пикере ссылок.
