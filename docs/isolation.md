...
закладок на панели и свои закреплённые вкладки, собранные из
`plain_context.bookmarks`
(`home/.chezmoiscripts/run_onchange_before_32-browser-extensions.sh.tmpl` и
`run_after_43-zen-session.sh.tmpl`). Общий список `bookmarks:` туда не едет —
это рабочие адреса. Цвет `cyan` намеренно вынут из ротации цветов рабочих
контекстов, чтобы `home` не совпал ни с одной работой, сколько бы их ни завели.
