...
скриптов этот документ разбирает только один — `20-packages`, потому что это
единственный скрипт, который выполняется на **каждой** установке независимо
от выбранных фич: он ставит собственно программы.
...
- Исходник chezmoi, тег `v2.71.1` (совпадает с `chezmoi --version` на этой
  машине): `internal/cmd/interactivetemplatefuncs.go`, функция
  `promptMultichoiceInteractiveTemplateFunc` (`strings.Split(valueStr, "/")`);
  `internal/cmd/prompt.go`, функции `readMultichoice`/`readLineRaw`
  (построчный ввод без TTY).
...
- [workarounds.md](workarounds.md) — реестр обходов; запись про
  `promptMultichoiceOnce` (другой механизм, не тот, что разобран здесь)
  лежит там.
