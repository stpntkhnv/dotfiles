...
|---|---|---|
| — (только хост) | `ssh -T git@github.com` отвечает «successfully authenticated» | «GitHub SSH: key not registered -- 'cat ~/.ssh/id_ed25519.pub', paste at https://github.com/settings/keys; if this machine was REINSTALLED, the old, already-registered keys are in the vault instead: 'ssh-restore -f host', then 'chezmoi apply' (docs/install.md)»; в контейнере этот шаг не показывается намеренно — совет зарегистрировать свежий ключ там вредит, см. строку ниже |
...
| `ziti` | `/opt/openziti/etc/identities/` не пуст | «OpenZiti: no identity -- 'sudo cp <your-identity>.json /opt/openziti/etc/identities/ && sudo systemctl start ziti-edge-tunnel' (the JSON comes from whoever runs your Ziti network; docs/network.md)» |
...
| `syncthing` | Путь хранилища `kb` совпадает с тем, что видит `claudefiles` (`~/.config/claudefiles/secrets.json`) | «Syncthing: claudefiles syncs the vault at … but the catalogue syncs … -- edit kb.vault_path in ~/.config/claudefiles/secrets.json to match the catalogue path, then rerun ~/.local/share/claudefiles/setup.sh» |
...
искать. Сгруппировано так же, как карта документации — [docs/README.md](README.md).
...
| Диктовка молчит: текст не появляется, ошибок нет | [voice.md](voice.md#когда-сломалось) |
| Микрофон не слышно — записывается тишина | [voice.md](voice.md#когда-сломалось) |
| Пишет не тот микрофон (встроенный вместо гарнитуры) | [voice.md](voice.md#когда-сломалось) |
| Микрофон занят другой программой | [voice.md](voice.md#когда-сломалось) |
