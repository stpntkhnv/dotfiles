...
| Multi-Account Containers | расширение Mozilla | UI контейнеров, **таблица «контейнер → прокси»**, применение прокси, `proxyDNS` |
...
1. **Управляет контейнерами.** Создание, имена, цвета.
2. **Держит таблицу «контейнер → прокси»** и применяет её. Ключ хранения
   `proxifiedContainersKey` в `browser.storage.local` расширения.
3. **Включает `proxyDNS`.** Отдельной галочки нет, расширение ставит его само
   для любого socks-прокси.
4. **Правила «Always Open This Site in…»** — привязка домена к контейнеру.

Настройка вручную, три шага и порядок важен:

**Шаг 1. Выдать право на прокси.** Manage Extension → Optional Permissions →
*Allow extension to control proxy settings*. Без этого поле прокси вообще не
появится.

**Шаг 2. Создать контейнеры** с именами точно как в `contexts:`, регистр
включительно: `digi3`, `stellium`, `personal`, `scratch`.

Почему побуквенно: `zen-open` подставляет имя в ссылку как есть, а расширение
`Open external links` ищет контейнер точным сравнением и **создаёт новый**, если
не нашло. Опечатка даёт настоящий контейнер **без прокси** и без единого
предупреждения.

**Шаг 3. Прописать прокси.** Редактирование контейнера → *Advanced proxy
settings*:
...
Пишется **`socks://`, а не `socks5://`**. Схему расширение разбирает
регуляркой `/(https?)|(socks4?)/`, и `socks5://` не совпадает ни с чем — прокси
молча отбрасывается. В Firefox `socks` уже означает SOCKS5.

Отсюда же следствие: опечатка в схеме стоит ещё и remote DNS, потому что
`proxyDNS` включается только для socks.
...
Дальше вручную: контейнер `newjob` в Multi-Account Containers, прокси
`socks://127.0.0.1:11084`, space с привязкой.
