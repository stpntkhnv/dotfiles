...
профиле лежали четыре расширения с AMO, а нашего не было ни в `extensions/`, ни в
`extensions.json`. В консоли браузера при этом:

    Download failed - ERROR_SIGNEDSTATE_REQUIRED - file:///usr/local/lib/zen-context-proxy.xpi

Ловушка в двух дефолтах. `MOZ_REQUIRE_SIGNING: false` в сборке Zen не отключает
проверку подписи, он лишь делает `xpinstall.signatures.required` обычным префом
вместо константы (`AddonSettings.sys.mjs:32-48`). А сам преф выставлен в `true`
дефолтами приложения (`browser/omni.ja -> defaults/preferences/firefox.js:27`),
которые перебивают `false` из toolkit-овского `greprefs.js`.

Поэтому политика Zen выставляет его сама:

    "Preferences": { "xpinstall.signatures.required": { "Value": false, "Status": "locked" } }

Политикой, а не через `user.js`: политика читается до создания профиля, поэтому
расширение приезжает при первом же запуске, а не после второго `apply`. `locked`,
чтобы значение не сбилось из `about:config` и не вернулось при обновлении Zen.
Ставить этот преф политике разрешено ровно потому, что `MOZ_REQUIRE_SIGNING`
выключен (`Policies.sys.mjs:2464-2466`): сборка, которая включит флаг, сделает
преф неустанавливаемым, и расширение снова перестанет грузиться. На этот случай
`run_after_zz-next-steps.sh` проверяет, что оно **загружено** — по `active` в
`extensions.json`, а не по наличию файла на диске.

Каталог приложения как обходной путь не годится: неподписанное там разрешено
(`MOZ_UNSIGNED_APP_SCOPE: true`), но сканирование каталогов вне профиля выключено
(`MOZ_ALLOW_ADDON_SIDELOAD: false`), а pacman стирал бы файл при каждом
обновлении Zen.
...
**Space Routing.** Четвёртый вход, и единственный, который не спрашивает.
Штатная фича Zen: правила «адрес содержит такой текст -> открыть в таком
пространстве», а пространство привязано к контейнеру, поэтому вместе с
пространством приезжает и прокси
(`ZenSpaceRoutingManager.sys.mjs:106-112`). Правила лежат в
`<профиль>/zen-space-routing.jsonlz4` и собираются из `contexts:` по флагу
`route`: `reference` это имя контекста, `matchType` это `contains`.

Сравнение идёт по **всему адресу**, а не только по хосту (`:304-340`), и это
единственное, чем правило лучше «Always Open This Site in…»: домен
`dev.azure.com/<организация>` покрывается путём, а не доменом. Всё остальное про
эти правила такое же, и главное — то же самое: правило **вытаскивает** ссылку.
Поэтому под ним только имена контекстов, только уникальные (`digi3`, `stellium`,
но не `personal` — слово встречается в обычных адресах), и никогда общие домены
Microsoft. Отказ выглядит так: чужая страница, в адресе которой случайно
оказалось имя контекста, уедет в его пространство и выйдет через его VPN.

Zen берёт первое совпадение в порядке массива (`routeUri`, `:276-295`), поэтому
дубль на один `reference` меняет поведение молча. Скрипт `43-zen-session` держит
свои правила по детерминированному id, обновляет их на месте, а рукописное
правило с тем же `reference` не трогает и говорит об этом вслух.
...
jq -r '[.addons[]|select(.active)|.id]|sort|.[]' ~/.config/zen/*/extensions.json  # что реально загружено
ls -l ~/.config/zen/*/zen-space-routing.jsonlz4   # правила маршрутизации на месте
```

Проверка идёт по `extensions.json`, а не по содержимому `extensions/`: наличие
`.xpi` не значит ничего, файл мог остаться от прошлой установки, да и у
отключённого расширения файл тоже на месте. Отвечает на вопрос поле `active`.
...
- `about:policies` → Active: должны быть все пять расширений — три контейнерных,
  Bitwarden и `context-proxy@dotfiles.local`.
